...
TEXT runtime·hwbpExceptionHandlerTrampoline(SB),NOSPLIT,$8-0
	// PExceptionPointers in CX from Windows VEH
	// Save it in our frame
	MOVQ	CX, 0(SP)
	...
	CMPQ	AX, $0
	JE	2(PC)
	// Called from Go thread, set R14.
	MOVQ	g(AX), R14

	// Reserve space for spill slots.
	ADJSP	$16
	
	// Get the exception pointers back from our frame
	// Our frame is at SP + 16 + REGS_HOST_TO_ABI0_STACK
	MOVQ	(16+REGS_HOST_TO_ABI0_STACK)(SP), AX
	
	// Call hwbpExceptionHandler with exception pointers in AX
	CALL	runtime·hwbpExceptionHandler<ABIInternal>(SB)
	// Return value is already in AX as int32.
	// Sign extend from int32 to int64 properly
